...
fences) freely _inside_ them. Do not nest XML deeper than needed â€” prefer flat, readable
...
written so an agent can decide _from this line alone_ whether the skill applies. Start with a
verb, name the concrete artifacts/tech, and end with "Use when â€¦".
...
| `<rules>`         | yes                            | The mandatory rules, grouped with `###` headings; include the _why_ where non-obvious. |
...
attention budget â€” cut redundancy, merge overlapping rules, delete anything that doesn't change
...
A one-clause rationale ("â€¦so streams can be torn down") makes the rule generalize. The model
applies it correctly to cases you didn't enumerate.

### Examples are pictures worth a thousand words

Include 1â€“5 curated, canonical examples. Make them relevant and diverse (cover the real shape,
not a laundry list of edge cases). Wrap each in `<example>`; show the _correct_ pattern.

### Don't over-prompt

Do not shout. Avoid "CRITICAL / YOU MUST / ALWAYS" stacked everywhere â€” current models
...
- When a project's real patterns diverge from a skill, fix the source skill here (or record why
  the project intentionally deviates), then re-sync â€” don't let the copy silently rot.
...
- [[clean-architecture-rn-expo-mvvm]] â€” the general architecture rules this format wraps.
- [[pr-checklist-clean-architecture]] â€” review gate that enforces several of these conventions.
  </see_also>
