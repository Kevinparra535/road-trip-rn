...
/**
 * Motor de sugerencias contextuales de navegación (estilo Waze/Google Maps):
 * a partir del progreso, el combustible, la elevación y la maniobra actual
 * decide qué avisos glanceables mostrar al motero en marcha — fuera de ruta,
 * tanqueo, gasolinera cerca, subida/bajada, curva cerrada, llegada — con un
 * ranking por prioridad y un lifecycle anti-parpadeo (tiempo mínimo visible +
 * cooldown).
 *
 * Lógica pura (sólo entidades de dominio + geoMath), sin infraestructura: vive
 * en `domain/useCases` como cualquier otra regla de negocio. El `HomeViewModel`
 * la orquesta y el `NavSuggestionRail` la pinta.
 */
...
export const buildNavigationSuggestions = ({
  ...
  if (nextFuelStop) {
    ...
    const station = findNearestFuelStation(nextFuelStop.location, fuelStations)?.station;
    const isFuelCritical = fuelEstimate !== null && !fuelEstimate.reachesWithoutRefuel;
    ...
    suggestions.push({
      ...
      title: isFuelCritical || isFuelSoon ? 'Tanqueo recomendado' : 'Proximo tanqueo',
      value: formatGuidanceDistance(remainingToStopKm),
      detail: station ? stationDisplayName(station) : 'Parada sugerida por autonomia',
  ...
  const hasFuelSuggestion = suggestions.some(
    (suggestion) => suggestion.kind === 'fuel' || suggestion.kind === 'fuel-warning',
  );
...
export const resolveNavigationSuggestionLifecycle = ({
  ...
  const nextCooldownUntilMsById = Object.fromEntries(
    Object.entries(previous.cooldownUntilMsById).filter(([, untilMs]) => untilMs > nowMs),
  );
  ...
  previous.visible.forEach((previousSuggestion) => {
    ...
    const enteredAtMs = previous.enteredAtMsById[previousSuggestion.id] ?? nowMs;
...
/**
 * Envoltura `UseCase` para la build (regla de negocio "qué avisos aplican").
 * El `resolveNavigationSuggestionLifecycle` es timing de presentación y se usa
 * como helper puro desde el ViewModel.
 */
@injectable()
export class BuildNavigationSuggestionsUseCase implements UseCase<
  NavigationGuidanceInput,
  NavSuggestion[]
> {
  async run(input: NavigationGuidanceInput): Promise<NavSuggestion[]> {
    return buildNavigationSuggestions(input);
  }
}
