...
import {
  ...
  BuildNavigationSuggestionsUseCase,
...
describe('buildNavigationSuggestions', () => {
  it('no sugiere nada mientras no se está navegando', () => {
    expect(buildNavigationSuggestions(baseInput({ isNavigating: false }))).toEqual([]);
  });
  ...
  it('avisa "autonomía justa" sin paradas cuando el combustible no alcanza', () => {
    const suggestions = buildNavigationSuggestions(
      baseInput({
        fuelStops: [],
        fuelEstimate: makeRouteFuelEstimate({
...
describe('resolveNavigationSuggestionLifecycle', () => {
...
describe('BuildNavigationSuggestionsUseCase', () => {
  it('run() delega en la función pura (misma salida)', async () => {
    const useCase = new BuildNavigationSuggestionsUseCase();
    const input = baseInput({ progressKm: 116.2, destinationName: 'Villa' });

    await expect(useCase.run(input)).resolves.toEqual(buildNavigationSuggestions(input));
  });

  it('run() devuelve [] cuando no se está navegando', async () => {
    const useCase = new BuildNavigationSuggestionsUseCase();

    await expect(useCase.run(baseInput({ isNavigating: false }))).resolves.toEqual([]);
  });
});

describe('formatGuidanceDistance', () => {
  it('formatea por tramos: ahora / metros / 1 decimal / entero', () => {
    expect(formatGuidanceDistance(0)).toBe('Ahora');
    expect(formatGuidanceDistance(0.04)).toBe('Ahora');
    expect(formatGuidanceDistance(0.4)).toBe('400 m');
    expect(formatGuidanceDistance(3.84)).toBe('3.8 km');
    expect(formatGuidanceDistance(42)).toBe('42 km');
  });
});
