...
| **F1b** | ✅ Hecho  | Runtime de nav (timer/reaction/getters/voz/arribo/off-route→reroute) reubicado a `NavigationSessionStore` singleton. `HomeViewModel` baja ~350 líneas y delega vía proxies (el `HomeScreen` no cambió). Tests del store + los 48 del VM verdes.                                                                                                                                                     |
...
| **F3**  | ✅ Código | Velocímetro real + reroute (F0/F1a) + **background completo en código**: `RequestBackgroundLocationUseCase`, `LocationService` background (BestForNavigation + foreground service), task + **registro headless→store** (`backgroundLocationRegistry` → `WatchBackgroundLocationUseCase` → `LocationStore`), flags en `app.json`. **Solo falta validar** la ejecución headless en development build. |
| **F4**  | ✅ Código | Topología/tipado cerrados en F0. `linking` tipado + auth-gating completo: `PendingDeepLinkStore`/`gateDeepLink` + `createAuthGatedLinking` (getInitialURL/subscribe) en `App.tsx` + `RootNavigator.resolvePending()`. **Solo falta validar** el cold-start de deep link en device.                                                                                                                  |
| **F5**  | ✅ Código | **via/stop** (`Waypoint.isVia`/`isStopPoint` + persistencia, G13). **rideStyle** plumbeado (`RideStyle` + `CalculateDirections.rideStyle` → exclude; curvy/fuel ≈ evitar autopistas — heurística, el curvy REAL es stack-blocked). **offline** (`OfflineMapRepository` + `DownloadOfflineCorridorUseCase`). **Falta**: UI selector/botón + validar descarga en device.                              |

**Resumen:** el **código de las 6 fases está implementado y verde** (832 tests). Lo único que
NO se puede cerrar aquí es, por naturaleza:

- **Validación en device** (Mapbox/background/offline/deep-link cold-start NO corren en Expo
  Go): background headless→store (F3), descarga offline (F5), cold-start de deep link (F4),
  posición del overlay (F2b). El código está escrito; falta un dev client en un teléfono.
- **UI triggers de F5**: selector de `rideStyle` y botón de descarga offline (la lógica
  domain/data está; el toggle visual es trabajo de UI con QA en device).
- **rideStyle "curvo" REAL / reroute offline**: **imposibles con el stack** (RNMapbox v10 +
  Directions API; el Mapbox Navigation SDK es producto aparte de pago — ver §0.2). Aproximados
  donde se puede (evitar autopistas) y documentados.
