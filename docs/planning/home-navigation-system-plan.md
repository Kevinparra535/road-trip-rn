...
| **F5**  | ✅ Código | **via/stop** (`Waypoint.isVia`/`isStopPoint` + persistencia, G13). **rideStyle** plumbeado (`RideStyle` + `CalculateDirections.rideStyle` → exclude) + **selector UI** (`RideStyleSelector` en el preview). **offline** (`OfflineMapRepository` + `DownloadOfflineCorridorUseCase`) + **botón** en RouteDetail. curvy/fuel ≈ evitar autopistas (heurística; el curvy REAL es stack-blocked). **Solo falta** validar la descarga en device. |

**Resumen:** el **código de las 6 fases está implementado y verde** (835 tests, 81 suites). NO
queda código por escribir desde este entorno. Lo único pendiente es, por naturaleza:
...
- **Validación en device** (Mapbox/background/offline/deep-link cold-start NO corren en Expo
  ...
  posición del overlay (F2b). El código está escrito; falta correr el dev client en un teléfono.
