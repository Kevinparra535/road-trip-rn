...
| **F3**  | 🟡 Parcial | Velocímetro real + reroute con reintentos (F0/F1a). **Background scaffolding hecho**: `RequestBackgroundLocationUseCase` + `LocationService.startBackgroundUpdates` (BestForNavigation + foreground service) + task `backgroundLocationTask.ts` + flags en `app.json` (UIBackgroundModes / Android bg+fg service). **Pendiente device**: la entrega headless→store y la validación en dev client. |
| **F4**  | 🟡 Parcial | Topología/tipado cerrados en F0. `linking` config tipado + tests. **Auth-gating hecho**: `PendingDeepLinkStore` + `gateDeepLink` (guarda el link con sesión cerrada y lo reemite al login) + `createAuthGatedLinking` (getInitialURL/subscribe) cableado en `App.tsx` + `RootNavigator.resolvePending()`. **Pendiente device**: el cold-start de deep link end-to-end.                            |
...
**Resumen:** todo lo **fully-verificable en este entorno** está hecho y verde — F0, F1a,
**F1b**, F2a, F2b, F4-config y F5-via/stop (lógica de dominio, VMs, stores, entidades,
tipado, parsing de linking, componentes). Lo que queda es, por su naturaleza, no
completable-y-verificable aquí:
